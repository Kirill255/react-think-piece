...
https://coursehunters.net/course/firebase-c-react-v2
...
Firebase considers empty strings to be values ​​too, so the `title != null` check doesn't work, you need to explicitly check `title != ""`
...
handleSubmit = async (event) => {
  ...
  try {
    ...
    user.updateProfile({ displayName }); // problem with error, needs reload the page
  } catch (error) {
    alert(error);
  }
